...
- iOS: FirebaseMessaging encaminha pelo APNs. O entitlement `aps-environment=production` está no arquivo de assinatura Release. O projeto Debug conserva a assinatura local sem capacidades, compatível com Personal Team.
...
3. Assinar o app iOS com Apple Developer Team e perfil que permitam Push Notifications. Para testar uma build Debug com push, usar um arquivo de entitlements com `aps-environment=development` e configurar `CODE_SIGN_ENTITLEMENTS` nessa build; o perfil Personal Team local não oferece essa capacidade.
