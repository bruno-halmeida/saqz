...
case " $(mobile_line "$happy_log") " in
    *' :features:access:allTests :features:groups:domain:allTests :features:groups:data:allTests :features:groups:compileAndroidMain :features:groups:allTests :navigation:allTests :compose-app:allTests '*) ;;
...
# The Nav3 navigation module owns the product route graph; its suite is mandatory.
case " $(mobile_line "$happy_log") " in
    *' :navigation:allTests '*) ;;
    *) printf 'mobile navigation suite missing\n' >&2; exit 1 ;;
esac
