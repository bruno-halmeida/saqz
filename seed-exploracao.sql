...
BEGIN
    ...
    SELECT
        ...
        now(),
        (ARRAY['PONTA', 'CENTRAL', 'OPOSTO', 'LEVANTADOR', 'LIBERO'])[1 + (atletas.n % 5)]::athlete_position,
        (CASE WHEN atletas.n <= 12 THEN 'MENSALISTA' ELSE 'AVULSO' END)::athlete_membership_type,
        atletas.n <> 20,
        -- Atenção: o nível do VÍNCULO é em PT-BR, o do GRUPO é em inglês. Não é engano daqui.
        (ARRAY['INICIANTE', 'INTERMEDIARIO', 'AVANCADO'])[1 + (atletas.n % 3)]::athlete_level,
        (ARRAY['DIREITA', 'ESQUERDA', 'TANTO_FAZ'])[1 + (atletas.n % 3)]::athlete_preferred_side,
        165 + (atletas.n % 25),
    ...
    SELECT
        ...
        status.valor::charge_status,
        (CASE WHEN status.valor = 'PAID' THEN (ARRAY['PIX', 'CASH'])[1 + (atletas.n % 2)] END)::charge_paid_method,
        v_owner,
        v_owner,
    ...
    SELECT
        ...
        despesa.categoria::expense_category,
        despesa.custom,
        despesa.nota,
        despesa.direcao::expense_direction,
        v_owner,
        v_owner,
