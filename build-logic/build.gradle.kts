plugins {
    `kotlin-dsl`
}

group = "br.com.saqz.buildlogic"

dependencies {
    implementation("org.jetbrains.kotlin:kotlin-gradle-plugin:2.4.10")
    implementation("org.jetbrains.compose:compose-gradle-plugin:1.11.1")
    implementation("com.android.tools.build:gradle:9.1.0")
}

gradlePlugin {
    plugins {
        register("jvmBackend") {
            id = "saqz.jvm-backend"
            implementationClass = "br.com.saqz.buildlogic.JvmBackendConventionPlugin"
        }
        register("kmpComposeLibrary") {
            id = "saqz.kmp-compose-library"
            implementationClass = "br.com.saqz.buildlogic.KmpComposeLibraryConventionPlugin"
        }
        register("androidApplication") {
            id = "saqz.android-application"
            implementationClass = "br.com.saqz.buildlogic.AndroidApplicationConventionPlugin"
        }
    }
}
