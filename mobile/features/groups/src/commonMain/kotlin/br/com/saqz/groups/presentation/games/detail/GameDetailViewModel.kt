...
class GameDetailViewModel(
    ...
    override fun onIntent(intent:GameDetailIntent){when(intent){
        ...
        GameDetailIntent.DismissConfirmation->if(!state.value.isMutating)update{state.value.copy(pendingAction=null)}
        GameDetailIntent.ConfirmLifecycle->confirm()
        is GameDetailIntent.RequestAttendance->requestAttendance(intent.action)
        GameDetailIntent.DismissAttendance->if(!state.value.isAttendanceMutating)update{state.value.copy(pendingAttendanceAction=null,attendanceCommandKey=null)}
        ...
        GameDetailIntent.CancelAttendanceImageShare->update{state.value.copy(showAttendanceSharePrivacy=false)}
    ...
    private fun load(){
        ...
        viewModelScope.launch{when(val result=gateway.read(GroupId(current.groupId),current.gameId)){
            is SaqzResult.Success->{update{state.value.copy(game=result.value.game,version=result.value.version,attendanceOpen=result.value.game.attendanceOpen(),error=null)};loadAttendance(current.groupId,current.gameId)}
            is SaqzResult.Failure->update{state.value.copy(game=null,version=null,isLoading=false,error=if(result.error==GameError.HiddenResource)GameDetailError.HIDDEN else GameDetailError.UNAVAILABLE)}
        }}
    }

    private suspend fun loadAttendance(groupId:String,gameId:String){
        val attendance=attendanceGateway
        if(attendance==null){update{state.value.copy(isLoading=false)};return}
        when(val result=attendance.read(GroupId(groupId),gameId)){
            is SaqzResult.Success->update{state.value.copy(attendance=result.value,isLoading=false,attendanceError=null,attendanceErrorMessage=null)}
            is SaqzResult.Failure->{val failure=result.error.toPresentation();update{state.value.copy(isLoading=false,attendanceError=failure.error,attendanceErrorMessage=failure.message)}}
        }
    }

    private fun refreshAttendance(){val current=state.value;if(current.isAttendanceMutating||attendanceGateway==null)return;viewModelScope.launch{loadAttendance(current.groupId,current.gameId)}}
    private fun openEdit(){val current=state.value;if(current.canEdit&&!current.isMutating)emit(GameDetailEffect.OpenEdit(current.groupId,current.gameId))}
    private fun request(action:GameLifecycleAction){val current=state.value;if(!current.isMutating&&action in current.actions)update{current.copy(pendingAction=action,error=null)}}
    private fun confirm(){val current=state.value;val version=current.version?:return;val action=current.pendingAction?:return;if(current.isMutating||action !in current.actions)return;update{current.copy(isMutating=true,error=null)};viewModelScope.launch{when(val result=gateway.lifecycle(GroupId(current.groupId),current.gameId,version,action.toDomain())){is SaqzResult.Success->{update{state.value.copy(game=result.value.game,version=result.value.version,isMutating=false,pendingAction=null,reloadAvailable=false)};emit(GameDetailEffect.LifecycleApplied(action))};is SaqzResult.Failure->fail(result.error)}}}
    ...
    private fun confirmAttendance(){val current=state.value;val action=current.pendingAttendanceAction?:return;val key=current.attendanceCommandKey?:return;execute(AttendanceOperation.Self(action,key))}
    private fun override(intent:GameDetailIntent.OverrideAttendance){val current=state.value;if(!current.organizer||current.isAttendanceMutating)return;if(intent.memberId.isBlank()||intent.reason.trim().length<2){update{current.copy(attendanceError=GameDetailError.VALIDATION,attendanceErrorMessage=null)};return};execute(AttendanceOperation.Override(intent.memberId,intent.intent,intent.reason.trim(),keys.create()))}
    private fun capacity(value:Int){val current=state.value;val version=current.version?:return;if(!current.organizer||current.isAttendanceMutating)return;if(value !in 2..100){update{current.copy(attendanceError=GameDetailError.VALIDATION,attendanceErrorMessage=null)};return};execute(AttendanceOperation.Capacity(value,AttendanceVersionToken(version.value),keys.create()))}
    ...
    private fun execute(operation:AttendanceOperation){
        ...
        update{current.copy(isAttendanceMutating=true,attendanceError=null,attendanceErrorMessage=null,retryAttendanceAvailable=false)}
        viewModelScope.launch{when(operation){
    ...
    private fun applied(result:AttendanceMutation,version:AttendanceVersionToken,updateOwn:Boolean){
        ...
        update{state.value.copy(attendance=detail,attendanceVersion=version,isAttendanceMutating=false,pendingAttendanceAction=null,attendanceCommandKey=null,attendanceError=null,attendanceErrorMessage=null,retryAttendanceAvailable=false).syncGame(detail)}
    ...
    private fun capacityApplied(result:VersionedAttendanceCapacity){
        retryOperation=null
        update{state.value.copy(attendance=result.value.detail,version=GameVersionToken(result.version.value),isAttendanceMutating=false,attendanceError=null,attendanceErrorMessage=null,retryAttendanceAvailable=false).syncGame(result.value.detail)}
        emit(GameDetailEffect.CapacityApplied(result.value.capacity,result.value.promotedCount))
    }
    private fun attendanceFailed(failure:AttendanceError){val mapped=failure.toPresentation();update{state.value.copy(isAttendanceMutating=false,attendanceError=mapped.error,attendanceErrorMessage=mapped.message,retryAttendanceAvailable=mapped.error in setOf(GameDetailError.CONFLICT,GameDetailError.UNAVAILABLE),reloadAvailable=mapped.error==GameDetailError.CONFLICT)}}
    private fun fail(failure:GameError){val error=when(failure){GameError.Conflict->GameDetailError.CONFLICT;GameError.HiddenResource->GameDetailError.HIDDEN;GameError.InvalidLifecycle->GameDetailError.INVALID_LIFECYCLE;else->GameDetailError.UNAVAILABLE};update{state.value.copy(isMutating=false,pendingAction=null,error=error,reloadAvailable=error==GameDetailError.CONFLICT||error==GameDetailError.INVALID_LIFECYCLE)}}
    private fun shareAttendanceLink(retry:Boolean){
        val current=state.value
        val url=current.attendanceLinkUrl
        if(retry&&url!=null){emit(GameDetailEffect.ShareAttendanceLink(AttendanceLinkUrl(url)));return}
        if(current.isAttendanceLinkLoading||!current.organizer||current.game?.status!=GameStatus.Published||!current.attendanceOpen)return
        val share=attendanceShareGateway?:return
        update{current.copy(isAttendanceLinkLoading=true,attendanceLinkError=null)}
        viewModelScope.launch{when(val result=share.rotateLink(GroupId(current.groupId),current.gameId)){
            is SaqzResult.Success->{update{state.value.copy(isAttendanceLinkLoading=false,attendanceLinkUrl=result.value.value,attendanceLinkError=null)};emit(GameDetailEffect.ShareAttendanceLink(result.value))}
            is SaqzResult.Failure->update{state.value.copy(isAttendanceLinkLoading=false,attendanceLinkError=GameDetailError.UNAVAILABLE)}
        }}
    }
    private fun requestAttendanceImageShare(){
        val current=state.value
        if(current.isAttendanceShareSnapshotLoading||!current.organizer)return
        if(current.attendanceShareSnapshot!=null){update{current.copy(showAttendanceSharePrivacy=true,attendanceShareError=null)};return}
        val share=attendanceShareGateway?:return
        update{current.copy(isAttendanceShareSnapshotLoading=true,attendanceShareError=null)}
        viewModelScope.launch{when(val result=share.readSnapshot(GroupId(current.groupId),current.gameId)){
            is SaqzResult.Success->{val image=AttendanceShareImage.from(result.value);update{state.value.copy(isAttendanceShareSnapshotLoading=false,attendanceShareSnapshot=image,attendanceShareError=null,showAttendanceSharePrivacy=true)}}
            is SaqzResult.Failure->update{state.value.copy(isAttendanceShareSnapshotLoading=false,attendanceShareError=GameDetailError.UNAVAILABLE)}
        }}
    }
    private fun confirmAttendanceImageShare(){val image=state.value.attendanceShareSnapshot?:return;update{state.value.copy(showAttendanceSharePrivacy=false)};emit(GameDetailEffect.ShareAttendanceImage(image))}
    private fun reportAttendanceShareResult(successful:Boolean){if(!successful&&state.value.attendanceShareSnapshot!=null)update{state.value.copy(attendanceShareError=GameDetailError.UNAVAILABLE)}}
