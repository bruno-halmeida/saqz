...
data class GameDetailState(
    ...
    val attendanceActions get()=if(!attendanceOpen||game?.status!=GameStatus.Published)emptySet() else when(ownAttendance?.status){null->setOf(AttendanceAction.CONFIRM,AttendanceAction.DECLINE);AttendanceStatus.Confirmed->setOf(AttendanceAction.WITHDRAW);AttendanceStatus.Declined->setOf(AttendanceAction.CONFIRM);AttendanceStatus.Waitlisted->setOf(AttendanceAction.WITHDRAW)}
}
