...
class GamesScreenTest {
    ...
    @Test fun `owner and admin see create action`()=runComposeUiTest{screen(base.copy(role=GroupRoleDto.ADMIN));onNodeWithTag(GamesTags.Create).assertExists()}
    @Test fun `athlete never sees organizer create action`()=runComposeUiTest{screen(base.copy(role=GroupRoleDto.ATHLETE));onNodeWithTag(GamesTags.Create).assertDoesNotExist()}
    ...
    @Test fun `status and waitlist labels never expose enum identifiers`()=runComposeUiTest{val games=listOf(item("draft",GameStatusDto.DRAFT,0,3),item("cancel",GameStatusDto.CANCELLED,0,0),item("done",GameStatusDto.COMPLETED,1,0));screen(base.copy(upcoming=games));onNodeWithText("Rascunho").assertExists();onNodeWithText("Lista de espera: 3").assertExists();onNodeWithText("DRAFT").assertDoesNotExist();onNodeWithText("America/Sao_Paulo").assertDoesNotExist()}
    ...
    @Test fun `compact viewport scrolls to every game card`()=runComposeUiTest{val many=(1..12).map{item("game-$it")};setContent{Box(Modifier.size(320.dp,420.dp)){SaqzTheme{GamesScreen(GamesScreenState(base.copy(upcoming=many))) {}}}};onNodeWithTag(GamesTags.card("game-12")).performScrollTo().assertIsDisplayed().assertHasClickAction()}
    ...
    private fun item(id:String="future",status:GameStatusDto=GameStatusDto.PUBLISHED,spots:Int=21,waitlist:Int=2)=GameListItem(id,"Treino semanal","12/08/2026","19:30","Arena Central",status,spots,waitlist,"2026-08-12T22:30:00Z")
    private val base get()=GamesState("group",GroupRoleDto.OWNER,listOf(item()),listOf(item("past")))
}
