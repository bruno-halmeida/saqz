...
class GamesViewModelTest {
    ...
    @Test fun `initial failure exposes error without protected content`()=runTest{val f=fixture(this);f.gateway.results[A]=SaqzResult.Failure(GameError.Data(DataError.Connectivity));f.vm.onIntent(select(A));runCurrent();assertEquals(GamesLoadError.UNAVAILABLE,f.vm.state.value.error);assertTrue(f.vm.state.value.upcoming.isEmpty())}
    ...
    @Test fun `organizer receives one create navigation effect`()=runTest{val f=fixture(this);f.vm.onIntent(select(A,GroupRole.ADMIN));runCurrent();val effect=async{f.vm.effects.first()};f.vm.onIntent(GamesIntent.OpenCreate);f.vm.onIntent(GamesIntent.OpenCreate);assertEquals(GamesEffect.OpenCreate(A),effect.await());assertNull(withTimeoutOrNull(1){f.vm.effects.first()})}
    @Test fun `athlete cannot open create`()=runTest{val f=fixture(this);f.vm.onIntent(select(A,GroupRole.ATHLETE));runCurrent();f.vm.onIntent(GamesIntent.OpenCreate);assertNull(withTimeoutOrNull(1){f.vm.effects.first()});assertFalse(f.vm.state.value.canCreate)}
    @Test fun `member receives one allowed game navigation effect`()=runTest{val f=fixture(this);f.vm.onIntent(select(A,GroupRole.ATHLETE));runCurrent();val effect=async{f.vm.effects.first()};f.vm.onIntent(GamesIntent.OpenGame("future"));f.vm.onIntent(GamesIntent.OpenGame("future"));assertEquals(GamesEffect.OpenGame(A,"future"),effect.await());assertNull(withTimeoutOrNull(1){f.vm.effects.first()})}
