...
class ExpenseScreenTest {
    ...
    @Test fun `preset category clears and hides custom value`()=runComposeUiTest{val intents=mutableListOf<ExpenseIntent>();screen(formState(),intents::add);onNodeWithTag(ExpenseTags.category(ExpenseCategory.Referee)).performScrollTo().performClick();val update=assertIs<ExpenseIntent.UpdateForm>(intents.single());assertEquals(ExpenseCategory.Referee,update.form.category);assertEquals("",update.form.customCategory);field("Nome da categoria").assertDoesNotExist()}
    ...
    @Test fun `voided expense hides mutation actions and keeps localized history`()=runComposeUiTest{val voided=expense().copy(status=ExpenseStatus.Voided,version=4,audit=expense().audit+ExpenseAudit("actor",ExpenseAction.Voided,"2026-08-13T10:00:00Z"));screen(state(expenses=listOf(voided)));onNodeWithText("Anulada").assertExists();onNodeWithText("Histórico: Anulada em 2026-08-13T10:00:00Z").assertExists();onNodeWithTag(ExpenseTags.edit(EXPENSE)).assertDoesNotExist();onNodeWithTag(ExpenseTags.void(EXPENSE)).assertDoesNotExist()}
    ...
    private fun formState()=state().copy(draft=ExpenseDraft(groupId=GROUP,commandKey="key",form=ExpenseForm("Água do jogo","123,45","2026-08-12",ExpenseCategory.Other,"Água","Compra manual")))
    private fun state(role:GroupRole=GroupRole.OWNER,expenses:List<Expense> = listOf(expense()))=ExpenseState(GROUP,role,expenses,FinanceTotals(2500,5000,1000,7000,12345),isLoading=false)
    private fun expense()=Expense(EXPENSE,GroupId(GROUP),"Água do jogo",12345,"2026-08-12",ExpenseCategory.Other,"Água","Compra manual",ExpenseStatus.Active,3,listOf(ExpenseAudit("actor",ExpenseAction.Created,"2026-08-12T10:00:00Z")))
