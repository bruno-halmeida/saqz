...
class AndroidGroupDraftStoreInstrumentedTest {
    ...
    @Test fun matchingSuccessAndGroupLossCleanupAreExact(){val adapters=AndroidGroupDraftAdapters.create(context);val one=ExpenseDraft(groupId=GROUP,commandKey=KEY,form=ExpenseForm("Água","12,34","2026-08-12",ExpenseCategory.Venue));val two=ExpenseDraft(groupId=OTHER,commandKey="other-key",form=ExpenseForm("Bola","20,00","2026-08-13",ExpenseCategory.Equipment));adapters.expense.write(one){};adapters.expense.write(two){};adapters.expense.clear(GROUP,null,"wrong"){};assertNotNull((adapters.store.readExpense(GROUP) as br.com.saqz.androidapp.groups.draft.AndroidDraftReadResult.Success).value);assertTrue(adapters.store.clearGroup(GROUP));assertSame(br.com.saqz.androidapp.groups.draft.AndroidDraftReadResult.Missing,adapters.store.readExpense(GROUP));assertEquals(OTHER,(adapters.store.readExpense(OTHER) as br.com.saqz.androidapp.groups.draft.AndroidDraftReadResult.Success).value.groupId)}
