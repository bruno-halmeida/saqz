...
class ChargeApprovalBackTest {
    ...
    @Test fun systemBackKeepsUnresolvedAttemptAndUsesReturnCallbackAfterResolution() {
        val marker = """{"actor":"owner","groupId":"group","chargeId":"charge","accountId":"account",""" +
            """"requestId":"request","fingerprint":"${"a".repeat(64)}"}"""
        ...
        val gateway = object : ChargeApprovalGateway {
            ...
            override suspend fun approve(target: ChargeApprovalTarget, command: ChargeApprovalCommand): Nothing =
                error("No new approval")
            override suspend fun cancel(target: ChargeApprovalTarget, orderId: String,
                requestId: String): Nothing = error("No new cancellation")
        }
        ...
        compose.waitForIdle()
        compose.runOnIdle { assertNull(saved.get<String>("approval.attempt")); assertEquals("order",
            vm.state.value.detail?.order?.id) }
        compose.runOnUiThread { compose.activity.onBackPressedDispatcher.onBackPressed() }
