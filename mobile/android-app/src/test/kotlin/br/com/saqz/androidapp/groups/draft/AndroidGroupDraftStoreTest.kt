...
class AndroidGroupDraftStoreTest {
    ...
    @Test fun expenseDraftRoundTripsConditionalFormVersionAndKey(){val f=fixture();val draft=expense();assertTrue(f.store.writeExpense(draft));assertEquals(draft,success(f.store.readExpense(GROUP)))}
    ...
    @Test fun corruptEnvelopeReturnsTypedCorruptWithoutDraft(){val f=fixture();f.store.writeExpense(expense());f.preferences.forceOnly("not-json");assertSame(AndroidDraftReadResult.Corrupt,f.store.readExpense(GROUP))}
    ...
    private fun game()=GameEditorDraft(groupId=GROUP,gameId=GAME,seriesId=null,commandKey=KEY,etag="\"4\"",mode=GameEditorMode.ONE_TIME,form=GameEditorForm("Treino",GameVenueDto(null,"Arena","Rua 1"),"2026-08-12","19:30:00","America/Sao_Paulo","2026-08-12T22:30:00Z","90","24","2026-08-12T19:00:00Z","25,00","Notas"))
    ...
    private companion object{const val GROUP="group-1";const val OTHER="group-2";const val GAME="game-1";const val EXPENSE="expense-1";const val KEY="draft-key"}
}
