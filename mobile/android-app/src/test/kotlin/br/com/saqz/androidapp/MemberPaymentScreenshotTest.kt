...
class MemberPaymentScreenshotTest {
    ...
    @Test fun historyStatesAndOrderSelection() {
        ...
        compose.runOnIdle { state.value = MemberPaymentHistoryState(loading = false, error = ReceiptError.NETWORK) }; capture("historico-erro")
        compose.runOnIdle { state.value = MemberPaymentHistoryState(loading = false, orders = listOf(order), nextCursor = order.id) }
    ...
    @Test fun paymentStatusesNeverMistakeRefundOrUncertaintyForSuccess() {
        ...
        val statuses = listOf("ACTIVE" to "Aguardando pagamento", "UNKNOWN" to "Pagamento em verificação",
            "CREATING" to "Pagamento em verificação", "CONFIRMED" to "Pagamento confirmado", "AVAILABLE" to "Pagamento confirmado",
            "REFUNDED" to "Pagamento reembolsado", "DISPUTED" to "Pagamento em contestação", "RECOVERY_PENDING" to "Pagamento em verificação",
        ...
        statuses.forEach { (status, label) ->
            compose.runOnIdle { state.value = active.copy(detail = MemberPaymentDetail(order, listOf(instrument.copy(status = status,
        ...
        compose.runOnIdle { state.value = active.copy(detail = MemberPaymentDetail(order.copy(status = "REFUNDED"), listOf(instrument.copy(status = "AVAILABLE")))) }
    ...
    @Test fun pixQrCopyFallbackAndHostedCardActions() {
        ...
        compose.runOnIdle { state.value = active.copy(detail = MemberPaymentDetail(order, listOf(instrument.copy(pixImage = "invalid")))) }
        ...
        compose.runOnIdle { state.value = active.copy(detail = MemberPaymentDetail(order, listOf(instrument.copy(quote = quote.copy(method = ReceiptMethod.CARD))))) }
        ...
        compose.onNodeWithText("Não foi possível abrir o pagamento. Tente novamente.").performScrollTo(); capture("cartao-erro-abertura")
    }
    ...
    private val review = MemberPaymentState(loading = false, detail = MemberPaymentDetail(order, emptyList()), method = ReceiptMethod.PIX,
        terms = ReceiptTerms("v1", "Termos de demonstração: confira o valor da cobrança, as taxas e o total antes de aceitar o pagamento."),
