...
class ChargeTest {
    ...
    @Test fun `paid records how the organizer received it`(){assertEquals(PaidMethod.CASH,ChargeTransitions.apply(charge(),ChargeStatusCommand(ChargeStatus.PAID,paidMethod=PaidMethod.CASH),UUID.randomUUID(),Instant.now(),UUID.randomUUID()).charge.paidMethod)}
    @Test fun `paid without a declared method is pix`(){assertEquals(PaidMethod.PIX,change(ChargeStatus.PAID).charge.paidMethod)}
    @Test fun `waived and cancelled record no payment method`(){listOf(ChargeStatus.WAIVED,ChargeStatus.CANCELLED).forEach{assertNull(change(it).charge.paidMethod)}}
