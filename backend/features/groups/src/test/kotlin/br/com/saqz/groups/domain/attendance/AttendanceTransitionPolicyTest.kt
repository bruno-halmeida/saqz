...
class AttendanceTransitionPolicyTest {
    ...
    @Test fun `organizer promotes a waitlisted member into a free spot`() = transition(organizer(current = AttendanceStatus.WAITLISTED, confirmed = 0), AttendanceIntent.CONFIRM, AttendanceStatus.CONFIRMED, changed = true, charge = true, reason = "Ajuste do organizador")
    @Test fun `organizer confirm on a full waitlist is denied not a silent no-op`() = denied(organizer(current = AttendanceStatus.WAITLISTED, confirmed = 2), AttendanceIntent.CONFIRM, AttendanceDenial.CAPACITY_FULL)
