...
class ChargeTransactionsTest{
    ...
    @Test fun `monthly rejects selection of a non member`(){val (service,repo)=service();repo.all=setOf(member);assertEquals(setOf("memberIds"),assertIs<MonthlyGenerationResult.Invalid>(service.generate(monthly(members=setOf(UUID.randomUUID())))).fields)}
    @Test fun `missing group is privacy hidden`(){val (service,repo)=service();repo.all=null;assertSame(MonthlyGenerationResult.Hidden,service.generate(monthly()))}
    @Test fun `valid monthly generation is stable ordered and transactional`(){val second=UUID.randomUUID();val (service,repo)=service();repo.all=setOf(member,second);val result=assertIs<MonthlyGenerationResult.Success>(service.generate(monthly(members=setOf(second,member))));assertEquals(setOf(member,second),result.charges.map{it.memberId}.toSet());assertEquals(1,repo.transactionCalls)}
    @Test fun `monthly excludes inactive members and reports how many were excluded`(){val second=UUID.randomUUID();val (service,repo)=service();repo.all=setOf(member,second);repo.inactive=setOf(second);val result=assertIs<MonthlyGenerationResult.Success>(service.generate(monthly(members=setOf(member,second))));assertEquals(listOf(member),result.charges.map{it.memberId});assertEquals(1,result.excludedInactiveCount)}
    @Test fun `monthly with only inactive members selected creates no charge and excludes all`(){val (service,repo)=service();repo.all=setOf(member);repo.inactive=setOf(member);val result=assertIs<MonthlyGenerationResult.Success>(service.generate(monthly(members=setOf(member))));assertEquals(emptyList(),result.charges);assertEquals(1,result.excludedInactiveCount)}
    ...
    private class FakeRepo:ChargeTransactionRepository{var gameCalls=0;var cancel:Triple<UUID,UUID,UUID>?=null;var all:Set<UUID>?=setOf(member);var inactive:Set<UUID> = emptySet();var transactionCalls=0;override fun createGameCharge(input:GameChargeInput,actorId:UUID,now:Instant):Charge{gameCalls++;return charge(input.memberId,ChargeIdentity.Game(input.gameId),requireNotNull(input.gameFeeCents),input.dueDate)};override fun reconcileGameCancellation(groupId:UUID,gameId:UUID,actorId:UUID,now:Instant){cancel=Triple(groupId,gameId,actorId)};override fun members(groupId:UUID)=all?.let{GroupMembers(it,it-inactive)};override fun createMonthlyCharge(command:MonthlyGenerationCommand,memberId:UUID,now:Instant)=charge(memberId,ChargeIdentity.Monthly(command.month),command.amountCents,command.dueDate);private fun charge(member:UUID,id:ChargeIdentity,amount:Long,due:LocalDate)=Charge(UUID.randomUUID(),group,member,id,amount,due)}
