...
class AttendanceControllerTest {
    ...
    @Test fun `self response after deadline is closed`() { actor = member; repository.deadline = NOW.minusSeconds(1); assertFailsWith<InvalidGameTransitionException> { controller.respond(ID, "$group", "$game", self()) } }
    @Test fun `cancelled game response is frozen`() { actor = member; repository.status = GameStatus.CANCELLED; assertFailsWith<InvalidGameTransitionException> { controller.respond(ID, "$group", "$game", self()) } }
    ...
    private inner class MemoryRepository : AttendanceCommandRepository, AttendanceDetailQuery {
        ...
        fun record(memberId: UUID, status: AttendanceStatus, sequence: Long? = null) { records[memberId] = AttendanceRecord(game, group, memberId, status, sequence, NOW, NOW, 1); if (sequence != null) allocator = maxOf(allocator, sequence) }
