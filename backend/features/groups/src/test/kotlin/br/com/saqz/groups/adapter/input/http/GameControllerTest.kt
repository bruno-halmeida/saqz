...
class GameControllerTest {
    ...
    @Test fun `edit accepts weak If-Match from a compressing proxy`() { val game=seed(); val response=controller.edit(ID,"$group","${game.id}","W/\"1\"",request().copy(requestId=null,title="Treino novo")); assertEquals("\"2\"",response.headers.eTag) }
