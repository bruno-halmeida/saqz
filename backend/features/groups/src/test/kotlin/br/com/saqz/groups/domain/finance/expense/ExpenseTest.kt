...
class ExpenseTest{
    ...
    @Test fun `confirmed category vocabulary contains racha and referee not service`(){assertEquals(setOf("VENUE","EQUIPMENT","REFEREE","RACHA","OTHER"),ExpenseCategory.entries.map{it.name}.toSet())}
    @Test fun `manual vocabulary has no monthly fee category`(){assertTrue(ExpenseCategory.entries.none{it.name in setOf("MONTHLY","MENSALIDADE","MONTHLY_FEE")})}
    @Test fun `omitted direction keeps the outflow the expenses always were`(){assertEquals(ExpenseDirection.OUT,valid(draft()).direction)}
    @Test fun `omitted direction on edit keeps the direction of the current snapshot`(){assertEquals(ExpenseDirection.IN,requireNotNull(ExpenseValidator.validate(draft(),ExpenseDirection.IN).snapshot).direction)}
    @Test fun `explicit direction always wins over the current one`(){assertEquals(ExpenseDirection.OUT,requireNotNull(ExpenseValidator.validate(draft(direction=ExpenseDirection.OUT),ExpenseDirection.IN).snapshot).direction)}
    @Test fun `racha entry is an inflow without custom category`(){val v=valid(draft(category=ExpenseCategory.RACHA,direction=ExpenseDirection.IN));assertEquals(ExpenseDirection.IN,v.direction);assertEquals(ExpenseCategory.RACHA,v.category);assertNull(v.customCategory)}
    ...
    private fun draft(description:String?=" Aluguel ",amount:Long?=5000,date:LocalDate?=LocalDate.of(2026,8,1),category:ExpenseCategory?=ExpenseCategory.VENUE,custom:String?=null,notes:String?=" Quadra ",direction:ExpenseDirection?=null)=ExpenseDraft(description,amount,date,category,custom,notes,direction)
}
