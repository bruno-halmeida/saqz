...
class ExpenseControllerTest{
    ...
    @Test fun `monthly fee is not a manual category because the charge already books it`(){listOf("MONTHLY","MENSALIDADE","MONTHLY_FEE").forEach{val e=assertFailsWith<InvalidGroupRequestException>{controller.create(ID,"$group",request().copy(category=it))};assertEquals(setOf("category"),e.fieldErrors.keys)}}
    @Test fun `omitted direction is the outflow of the old contract`(){assertEquals("OUT",controller.create(ID,"$group",request()).body!!.direction)}
    @Test fun `racha entry is created as an inflow`(){val response=controller.create(ID,"$group",request().copy(category="RACHA",direction="IN")).body!!;assertEquals("IN",response.direction);assertEquals("RACHA",response.category)}
    @Test fun `unknown direction is validation not server error`(){val e=assertFailsWith<InvalidGroupRequestException>{controller.create(ID,"$group",request().copy(direction="SIDEWAYS"))};assertEquals(setOf("direction"),e.fieldErrors.keys)}
    ...
    @Test fun `active inflow entry never inflates the expense total`(){controller.create(ID,"$group",request());controller.create(ID,"$group",request().copy(description="Racha do sábado",amountCents=12000,category="RACHA",direction="IN"));val list=controller.list(ID,"$group");assertEquals(2,list.expenses.size);assertEquals(5000,list.activeExpenseTotalCents);assertEquals(5000,controller.totals(ID,"$group").activeExpenseCents)}
