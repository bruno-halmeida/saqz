...
class FinanceMigrationIntegrationTest {
    ...
    /** Cobrança recebida antes da V32 não transita de novo, então o default de produto precisa alcançar o histórico na própria migração. */
    @Test fun `v32 backfills already paid charges with the legacy pix default`(){flyway().clean();flyway("31").migrate();val f=fixture("backfill");val paid=charge(f,"MONTHLY",month="2026-08-01");execute("UPDATE group_charges SET status='PAID' WHERE id='$paid'");val pending=charge(f,"MONTHLY",month="2026-09-01");flyway().migrate();assertEquals("PIX",string("SELECT paid_method FROM group_charges WHERE id='$paid'"));assertEquals(null,string("SELECT paid_method FROM group_charges WHERE id='$pending'"))}
    @Test fun `v32 leaves waived and cancelled charges without a method because they were never received`(){flyway().clean();flyway("31").migrate();val f=fixture("never-received");val ids=listOf("WAIVED","CANCELLED").mapIndexed{index,status->val id=charge(f,"MONTHLY",month="2026-0${index+1}-01");execute("UPDATE group_charges SET status='$status' WHERE id='$id'");id};flyway().migrate();ids.forEach{assertEquals(null,string("SELECT paid_method FROM group_charges WHERE id='$it'"))}}
    @Test fun `v32 reads legacy expenses as outflows`(){flyway().clean();flyway("31").migrate();val f=fixture("legacy-expense");val e=expense(f);flyway().migrate();assertEquals("OUT",string("SELECT direction FROM group_expenses WHERE id='$e'"))}

    private fun fixture(subject:String="finance"):Fixture{val actor=user("$subject-owner");val member=user("$subject-member");val group=UUID.randomUUID();execute("INSERT INTO access_groups (id,owner_user_id,creation_key,name,time_zone,profile_status,modality,composition,created_at,updated_at) VALUES ('$group','$actor','${UUID.randomUUID()}','Finance Group','America/Sao_Paulo','COMPLETE','COURT_VOLLEYBALL','MIXED',now(),now())");execute("INSERT INTO group_memberships (group_id,user_id,role,created_at,updated_at) VALUES ('$group','$member','ATHLETE',now(),now())");val game=UUID.randomUUID();execute("INSERT INTO games (id,group_id,title,local_date,local_time,zone_id,starts_at,duration_minutes,confirmation_deadline,venue_name,venue_address,capacity,game_fee_cents,status,created_at,updated_at) VALUES ('$game','$group','Treino',DATE '2026-08-12',TIME '19:30','America/Sao_Paulo',TIMESTAMPTZ '2026-08-12 22:30Z',90,TIMESTAMPTZ '2026-08-11 22:30Z','Arena','Rua Central 100',24,2500,'PUBLISHED',now(),now())");return Fixture(group,actor,member,game)}
    ...
    private fun flyway(target:String?=null)=Flyway.configure().dataSource(dataSource).locations(*allGroupFeatureMigrationLocations()).cleanDisabled(false).also{if(target!=null)it.target(target)}.load()
