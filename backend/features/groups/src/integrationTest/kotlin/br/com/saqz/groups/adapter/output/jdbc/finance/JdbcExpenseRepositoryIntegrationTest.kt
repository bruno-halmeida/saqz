...
class JdbcExpenseRepositoryIntegrationTest{
    ...
    @Test fun `deleted group hides expenses and rejects creation`(){val f=fixture();execute("UPDATE access_groups SET deleted_at=now() WHERE id='${f.group}'");assertSame(ExpenseResult.Hidden,f.service.list(f.owner,f.group));assertSame(ExpenseResult.Hidden,f.service.create(f.owner,f.group,draft()));assertEquals(0,int("SELECT count(*) FROM group_expenses"))}
