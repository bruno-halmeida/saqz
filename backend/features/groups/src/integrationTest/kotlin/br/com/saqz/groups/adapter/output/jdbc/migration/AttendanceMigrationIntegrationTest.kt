...
class AttendanceMigrationIntegrationTest {
    ...
    @Test fun `departed waitlist sequence can be reused to restore an original position`() { val f = fixture(); attendance(f, "WAITLISTED", 1); execute("UPDATE game_attendance SET status='DECLINED',waitlist_sequence=NULL,version=2,updated_at=now() WHERE game_id='${f.game}' AND member_user_id='${f.member}'"); val second = member(f.group, "second"); attendance(f.copy(member = second), "WAITLISTED", 2); val third = member(f.group, "third"); attendance(f.copy(member = third), "WAITLISTED", 1); assertEquals(2, int("SELECT waitlist_sequence_allocator FROM games WHERE id='${f.game}'")) }
