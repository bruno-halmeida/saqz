...
class JdbcAttendanceCommandRepositoryIntegrationTest {
    ...
    @Test fun `organizer confirms a self waitlisted avulso with audit and charge`() { val f = fixture(fee = 2500); val avulso = member(f.group, "avulso", "AVULSO"); success(f.service.execute(avulso, f.group, f.game, intent = AttendanceIntent.CONFIRM)); assertEquals("WAITLISTED", status(avulso)); val saved = success(f.service.execute(f.owner, f.group, f.game, avulso, AttendanceIntent.CONFIRM, AttendanceSource.ORGANIZER, "Autorizado pelo organizador")); assertEquals(AttendanceStatus.CONFIRMED, saved.status); assertEquals(1, count("group_charges")); assertEquals(2, count("attendance_events")) }
    @Test fun `organizer confirm on a full waitlist is explicitly denied`() { val f = fullFixture(); val waiting = member(f.group, "waiting"); waitlist(f, waiting, 1); val result = f.service.execute(f.owner, f.group, f.game, waiting, AttendanceIntent.CONFIRM, AttendanceSource.ORGANIZER, "Sem vaga"); assertEquals(AttendanceCommandResult.Denied(AttendanceDenial.CAPACITY_FULL), result); assertEquals("WAITLISTED", status(waiting)) }
    @Test fun `undo restores an automatic promotion to its original waitlist position`() { val f = promotionFixture(); f.service.execute(f.member, f.group, f.game, intent = AttendanceIntent.DECLINE); assertEquals("CONFIRMED", status(f.waiting.first())); val result = assertIs<UndoPromotionResult.Success>(undo().execute(f.owner, f.group, f.game, f.waiting.first())); assertEquals(AttendanceStatus.WAITLISTED, result.attendance.status); assertEquals(1L, result.attendance.waitlistSequence); assertEquals("WAITLISTED", status(f.waiting.first())); assertEquals(1, int("SELECT waitlist_sequence FROM game_attendance WHERE member_user_id='${f.waiting.first()}'")); assertEquals("WAITLISTED", status(f.waiting.last())); assertEquals(2, int("SELECT waitlist_sequence FROM game_attendance WHERE member_user_id='${f.waiting.last()}'")) }
    @Test fun `undo of a capacity increase promotion restores original waitlist position`() { val f = fullFixture(); val waiting = (1..2).map { member(f.group, "waiting-$it").also { id -> waitlist(f, id, it.toLong()) } }; capacity(f).execute(f.owner, f.group, f.game, 1, 3); assertEquals("CONFIRMED", status(waiting.first())); val result = assertIs<UndoPromotionResult.Success>(undo().execute(f.owner, f.group, f.game, waiting.first())); assertEquals(1, int("SELECT waitlist_sequence FROM game_attendance WHERE member_user_id='${waiting.first()}'")) }
    @Test fun `undo is refused for a manual organizer promotion`() { val f = fixture(); val avulso = member(f.group, "avulso", "AVULSO"); success(f.service.execute(f.owner, f.group, f.game, avulso, AttendanceIntent.CONFIRM, AttendanceSource.ORGANIZER, "Autorizado pelo organizador")); assertSame(UndoPromotionResult.NotAutomaticPromotion, undo().execute(f.owner, f.group, f.game, avulso)) }
    @Test fun `undo is refused for a member who was never promoted`() { val f = fixture(); success(f.service.execute(f.member, f.group, f.game, intent = AttendanceIntent.CONFIRM)); assertSame(UndoPromotionResult.NotAutomaticPromotion, undo().execute(f.owner, f.group, f.game, f.member)) }
    @Test fun `athlete cannot undo a promotion`() { val f = promotionFixture(); f.service.execute(f.member, f.group, f.game, intent = AttendanceIntent.DECLINE); assertSame(UndoPromotionResult.Forbidden, undo().execute(f.member, f.group, f.game, f.waiting.first())) }
    @Test fun `undo of an unknown member is privacy hidden`() { val f = fixture(); assertSame(UndoPromotionResult.Hidden, undo().execute(f.owner, f.group, f.game, user("outsider"))) }
    ...
    private fun undo() = UndoAutomaticPromotion(JdbcTransactionRunner(dataSource), JdbcAttendanceCommandRepository(dataSource), { NOW })
    ...
    private fun member(group: UUID, subject: String, membership: String = "MENSALISTA"): UUID { val id = user(subject); execute("INSERT INTO group_memberships (group_id,user_id,role,created_at,updated_at,membership_type) VALUES ('$group','$id','ATHLETE',now(),now(),'$membership')"); return id }
