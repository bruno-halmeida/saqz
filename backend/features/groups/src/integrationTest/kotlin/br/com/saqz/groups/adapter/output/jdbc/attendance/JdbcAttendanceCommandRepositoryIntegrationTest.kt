...
class JdbcAttendanceCommandRepositoryIntegrationTest {
    ...
    @Test fun `organizer override records the target member display name snapshot`() { val f = fixture(); val target = member(f.group, "target", "Alvo"); success(f.service.execute(f.owner, f.group, f.game, target, AttendanceIntent.CONFIRM, AttendanceSource.ORGANIZER, "Chegou após o prazo")); assertEquals("Alvo", snapshotName(target)) }
    @Test fun `withdrawal promotion keeps the snapshot taken when the member joined the queue`() { val f = fixture(capacity = 2); attendance(f, member(f.group, "occupied"), "CONFIRMED"); success(f.service.execute(f.member, f.group, f.game, intent = AttendanceIntent.CONFIRM)); val waiting = member(f.group, "waiting", "Espera"); assertEquals(AttendanceStatus.WAITLISTED, success(f.service.execute(waiting, f.group, f.game, intent = AttendanceIntent.CONFIRM)).status); execute("UPDATE access_users SET display_name='Renomeado' WHERE id='$waiting'"); f.service.execute(f.member, f.group, f.game, intent = AttendanceIntent.DECLINE); assertEquals("CONFIRMED", status(waiting)); assertEquals("Espera", snapshotName(waiting)) }
    @Test fun `capacity promotion keeps the snapshot taken when the member joined the queue`() { val f = fullFixture(); val waiting = member(f.group, "waiting", "Espera"); assertEquals(AttendanceStatus.WAITLISTED, success(f.service.execute(waiting, f.group, f.game, intent = AttendanceIntent.CONFIRM)).status); execute("UPDATE access_users SET display_name='Renomeado' WHERE id='$waiting'"); assertIs<CapacityCommandResult.Success>(capacity(f).execute(f.owner, f.group, f.game, 1, 3)); assertEquals("CONFIRMED", status(waiting)); assertEquals("Espera", snapshotName(waiting)) }
    ...
    private fun member(group: UUID, subject: String, name: String = "User"): UUID { val id = user(subject, name); execute("INSERT INTO group_memberships VALUES ('$group','$id','ATHLETE',now(),now())"); return id }
