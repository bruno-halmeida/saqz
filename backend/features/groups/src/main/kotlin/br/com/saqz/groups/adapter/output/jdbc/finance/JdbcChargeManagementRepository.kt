...
class JdbcChargeManagementRepository(dataSource:DataSource):ChargeManagementRepository{
    ...
    override fun update(change:ChargeChange,expectedVersion:Long):Boolean{val c=change.charge;val updated=jdbc.sql("UPDATE group_charges SET status=:status,changed_by_user_id=:actor,version=version+1,updated_at=:now WHERE id=:id AND group_id=:group AND version=:version AND EXISTS (SELECT 1 FROM access_groups g WHERE g.id=:group AND g.deleted_at IS NULL)").param("status",c.status.name).param("actor",change.event.actorId).param("now",Timestamp.from(change.event.occurredAt)).param("id",c.id).param("group",c.groupId).param("version",expectedVersion).update();if(updated!=1)return false;val e=change.event;val eventInserted=jdbc.sql("INSERT INTO group_charge_events (id,charge_id,group_id,actor_user_id,old_status,new_status,note,occurred_at) SELECT :id,:charge,:group,:actor,:old,:new,:note,:now WHERE EXISTS (SELECT 1 FROM access_groups g WHERE g.id=:group AND g.deleted_at IS NULL)").param("id",e.id).param("charge",e.chargeId).param("group",c.groupId).param("actor",e.actorId).param("old",e.oldStatus.name).param("new",e.newStatus.name).param("note",e.note,java.sql.Types.VARCHAR).param("now",Timestamp.from(e.occurredAt)).update();check(eventInserted==1){"Grupo de cobrança excluído ou inexistente"};return true}
    private fun events(charge:UUID)=jdbc.sql("SELECT events.id,events.charge_id,events.actor_user_id,events.old_status,events.new_status,events.note,events.occurred_at FROM group_charge_events events JOIN access_groups groups ON groups.id=events.group_id AND groups.deleted_at IS NULL WHERE events.charge_id=:charge ORDER BY events.occurred_at,events.id").param("charge",charge).query{rs,_->ChargeEvent(rs.getObject("id",UUID::class.java),rs.getObject("charge_id",UUID::class.java),rs.getObject("actor_user_id",UUID::class.java),ChargeStatus.valueOf(rs.getString("old_status")?:"PENDING"),ChargeStatus.valueOf(rs.getString("new_status")),rs.getString("note"),rs.getTimestamp("occurred_at").toInstant())}.list()
    ...
    private companion object{const val ROLE="""SELECT CASE WHEN g.owner_user_id=:actor THEN 'OWNER' ELSE m.role END FROM access_groups g LEFT JOIN group_memberships m ON m.group_id=g.id AND m.user_id=:actor WHERE g.id=:group AND g.deleted_at IS NULL AND (g.owner_user_id=:actor OR m.user_id IS NOT NULL)""";const val SELECT_CHARGE="SELECT c.id,c.group_id,c.member_user_id,c.game_id,c.billing_month,c.amount_cents,c.due_date,c.status,c.version,c.review_required FROM group_charges c JOIN access_groups g ON g.id=c.group_id AND g.deleted_at IS NULL"}
}
