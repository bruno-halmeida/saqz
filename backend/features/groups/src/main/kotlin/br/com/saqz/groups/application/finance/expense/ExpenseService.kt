...
class ExpenseService(private val tx:TransactionRunner,private val repository:ExpenseRepository,private val ids:()->UUID,private val now:()->Instant){
    // O total é de DESPESA: entrada avulsa (IN) não infla o tile, ela é saldo, e saldo é o VUL-176.
    fun list(actor:UUID,group:UUID):ExpenseResult=authorized(actor,group){val rows=repository.list(group);ExpenseResult.Listed(ExpenseList(rows,rows.filter{it.expense.status==ExpenseStatus.ACTIVE&&it.expense.snapshot.direction==ExpenseDirection.OUT}.sumOf{it.expense.snapshot.amountCents}))}
    fun create(actor:UUID,group:UUID,draft:ExpenseDraft,expenseId:UUID=ids()):ExpenseResult=tx.inTransaction{authorized(actor,group){val valid=ExpenseValidator.validate(draft);if(valid.fields.isNotEmpty())return@authorized ExpenseResult.Invalid(valid.fields);val expense=Expense(expenseId,group,requireNotNull(valid.snapshot));val event=expense.event(actor,ExpenseAction.CREATED,ids(),now());if(!repository.create(expense,event))repository.find(group,expenseId)?.let(ExpenseResult::Saved)?:ExpenseResult.Conflict else ExpenseResult.Saved(ExpenseWithEvents(expense,listOf(event)))}}
    fun edit(actor:UUID,group:UUID,id:UUID,version:Long,draft:ExpenseDraft):ExpenseResult=tx.inTransaction{authorized(actor,group){val current=repository.find(group,id)?:return@authorized ExpenseResult.Hidden;if(current.expense.version!=version)return@authorized ExpenseResult.Conflict;if(current.expense.status!=ExpenseStatus.ACTIVE)return@authorized ExpenseResult.InvalidLifecycle;val valid=ExpenseValidator.validate(draft,current.expense.snapshot.direction);if(valid.fields.isNotEmpty())return@authorized ExpenseResult.Invalid(valid.fields);val changed=current.expense.copy(snapshot=requireNotNull(valid.snapshot),version=version+1);val event=changed.event(actor,ExpenseAction.EDITED,ids(),now());if(!repository.update(changed,event,version))ExpenseResult.Conflict else ExpenseResult.Saved(ExpenseWithEvents(changed,current.events+event))}}
