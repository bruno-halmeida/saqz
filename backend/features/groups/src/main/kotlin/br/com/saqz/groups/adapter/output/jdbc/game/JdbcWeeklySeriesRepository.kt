...
class JdbcWeeklySeriesRepository(private val dataSource: DataSource) : WeeklySeriesRepository {
    ...
    private companion object {
        const val ROLE="SELECT CASE WHEN g.owner_user_id=? THEN 'OWNER' ELSE m.role END FROM access_groups g LEFT JOIN group_memberships m ON m.group_id=g.id AND m.user_id=? WHERE g.id=? AND g.deleted_at IS NULL AND (g.owner_user_id=? OR m.user_id=?)"
