...
class JdbcWeeklySeriesRepository(private val dataSource: DataSource) : WeeklySeriesRepository {
    ...
    private companion object {
        const val ROLE="SELECT CASE WHEN g.owner_user_id=? THEN 'OWNER' ELSE m.role::text END FROM access_groups g LEFT JOIN group_memberships m ON m.group_id=g.id AND m.user_id=? WHERE g.id=? AND g.deleted_at IS NULL AND (g.owner_user_id=? OR m.user_id=?)"
        ...
        const val INSERT_SLOT="/* Grupo ativo bloqueado em create() */ INSERT INTO game_series_slots (series_revision_id,group_id,slot_key,title,weekday,local_time,duration_minutes,venue_id,venue_name,venue_address,venue_court,capacity,confirmation_lead_minutes,game_fee_cents,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,now())"
