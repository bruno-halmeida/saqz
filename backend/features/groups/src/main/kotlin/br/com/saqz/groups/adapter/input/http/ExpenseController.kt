...
@RestController class ExpenseController(private val actors:VerifiedGroupActorResolver,private val expenses:ExpenseService,private val charges:ChargeManagement){
    ...
    // Categoria de mensalidade não existe no vocabulário manual: mensalidade só entra por charge, senão duplica o caixa.
    private fun ExpenseRequest.draft()=ExpenseDraft(description,amountCents,expenseDate,category?.let{runCatching{ExpenseCategory.valueOf(it)}.getOrNull()},customCategory,notes,direction?.let{runCatching{ExpenseDirection.valueOf(it)}.getOrNull()?:invalid("direction")});private fun uuid(v:String)=runCatching{UUID.fromString(v)}.getOrElse{throw GameNotFoundException()};private fun version(v:String?):Long{if(v==null)throw PreconditionRequiredException();return Regex("(?:W/)?\"([1-9][0-9]*)\"").matchEntire(v.trim())?.groupValues?.get(1)?.toLong()?:invalid("ifMatch")};private fun invalid(field:String):Nothing=throw InvalidGroupRequestException(mapOf(field to listOf("is required or invalid")))
}
private fun ExpenseWithEvents.response():ExpenseResponse{val e=expense;val s=e.snapshot;return ExpenseResponse(e.id,e.groupId,s.description,s.amountCents,s.expenseDate,s.category.name,s.customCategory,s.notes,s.direction.name,e.status.name,e.version,events.map{ExpenseEventResponse(it.actorId,it.action.name,it.occurredAt)})}
