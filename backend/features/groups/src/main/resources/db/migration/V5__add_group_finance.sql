...
CREATE TABLE group_expenses (
    ...
    CONSTRAINT ck_group_expenses_category CHECK (category IN ('VENUE', 'EQUIPMENT', 'SERVICE', 'OTHER')),
    CONSTRAINT ck_group_expenses_custom CHECK ((category = 'OTHER' AND custom_category IS NOT NULL AND custom_category = btrim(custom_category) AND char_length(custom_category) BETWEEN 2 AND 80 AND custom_category !~ '[[:cntrl:]]') OR (category <> 'OTHER' AND custom_category IS NULL)),
