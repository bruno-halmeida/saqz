...
class AdminReceivableConditionsEndpointIntegrationTest {
    ...
    @BeforeEach fun reset() { publisher.requests.clear() }
    ...
    fun `publication binds exact money amounts and attributes changes to authenticated admin`() {
        ...
        for (invalid in listOf("{}", fees(UUID.randomUUID()).replace("\"providerFixedCents\":39", "\"providerFixedCents\":39.5"),
            fees(UUID.randomUUID()).replace("\"providerRate\":0.0299", "\"providerRate\":1"))) {
    ...
    private fun fees(id: UUID) = """{"requestId":"$id","method":"PIX","providerRate":0.0299,"providerFixedCents":39,"commissionRate":0.02,"commissionFixedCents":100,"termsVersion":"v1","effectiveAt":"2026-09-13T00:00:00Z","baseCents":10000}"""
    ...
    class Config {
        ...
        @Bean fun controller(admins: PlatformAdminLookup, publisher: RecordingPublisher) =
