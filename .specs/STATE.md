...
- **Phase / Task**: Execute — Phase 6 / T42 (game detail lifecycle route).
...
- **In-progress** (file:line): `.specs/features/group-management/tasks.md:1190` — implement the member game-detail lifecycle route.
- **Next step**: Execute T42 with authoritative detail loading, role/status-gated organizer actions, versioned lifecycle confirmation, conflict reload, and read-only terminal states.
