...
| B31 | 2026-07-22 | The first finance data mapper constructed `ValidationDetails` with field messages only, omitting its required global-message list. | No new §V invariant: T32's focused multiplatform compilation already detects incomplete typed-validation construction; preserve an empty global list when the transport problem supplies only field errors. |
| B32 | 2026-07-22 | The migrated finance MockEngine fixture omitted the explicit common `respond` extension import, so Kotlin/Native test compilation could not resolve its response helpers. | No new §V invariant: T32's focused multiplatform test compilation already detects missing common-test imports; import MockEngine response helpers explicitly. |
| B33 | 2026-07-22 | The charge ViewModel migration reused `groupId` for both its raw constructor string and opaque domain identifier, so property initialization resolved the raw value at the state call site. | No new §V invariant: T33's focused multiplatform production compilation detects shadowed boundary values; name raw constructor inputs explicitly before wrapping them. |
