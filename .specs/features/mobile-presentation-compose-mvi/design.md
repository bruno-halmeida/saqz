...
5. **Form state into ViewModels + restoration** (PMVI-002/018–020): the five form screens (GroupSetup, Expense, Finance, GameDetail, GameEditor) hold input in `remember` today; input that affects validation/submission moves into ViewModel state via typed intents. Essential fields back onto `SavedStateHandle` (add `lifecycle-viewmodel-savedstate`, org.jetbrains.androidx). Where a durable draft exists (Expense drafts repo), the draft stays the source of truth and SavedStateHandle only carries the identifiers needed to re-load it (PMVI-020).
...
| Drafts mechanism | `DraftsModule`, Expense `draft.commandKey` | Source of truth for Expense restoration (PMVI-020); SavedStateHandle stores only reload keys |
...
- **Reuses**: Expense reconciles with the drafts repo (draft wins, PMVI-020); restored input re-validates through the existing validation path, never auto-submits (PMVI-019)
