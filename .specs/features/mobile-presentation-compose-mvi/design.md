...
| God-VM split granularity | Per navigable destination, orchestrators retained | PMVI-001 reads per-route; AD-025's "panels share the route ViewModel" exception applies only until destinations are real navigation entries. **Correction (2026-07-23, during T22 impl):** the five pre-auth routes (Login/Registration/PasswordReset/Verification/NameCompletion) ARE distinct destinations (driven by `SessionAccessStateMachine`) and were split (T17–T21). But Settings/Memberships/Invite proved to be state-derived panels of the authenticated group-context — they share `AccessRouteState` and the group-administration `runtime`, and are exactly the case AD-025 exempts. They are DEFERRED (not split) until AD-029/Nav3 promotes them to real entries. Phase 5's GroupsList/GroupDetail/GroupMore must be re-checked against the same test before splitting. |
